...
void two_checker(pair<int, int> x, pair<int, int> y, int n, int m, vector< pair<int, int> > points, vector< pair< pair<int, int>, pair<int, int> > > &point_pairs){
    bool found = false;
...
int main(){
    ...
    for (int i = 0; i < point_pairs.size(); i++){
        ...
        if(dist(x, y) > 1 && (x.first <= y.first) && (x.second >= y.second) && (x.second - y.second) >= (y.first - x.first)){
            two_checker(x, y, n, m, points, point_pairs);
        }
    }
}